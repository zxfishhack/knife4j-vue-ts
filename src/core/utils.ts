...
class KUtils {
   ...
   * @deprecated The method should not be used
   */
  getClassName (item:string, oas2:boolean) {
    if (oas2) {
  ...
  getRefParameterName (item:string) {
  ...
  toString (obj:string, defutStr:string) {
    let str = defutStr
    if (obj != undefined && typeof (obj) !== 'undefined' && obj != null) {
      str = obj.toString()
    }
  ...
  replaceMultipLineStr (str:string) {
  ...
  generUUID () {
    return (this.randomNumber() + this.randomNumber() + '-' + this.randomNumber() + '-' + this.randomNumber() + '-' + this.randomNumber() + '-' + this.randomNumber() + this.randomNumber() + this.randomNumber())
  }
