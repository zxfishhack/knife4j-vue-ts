...
export default class SwaggerBootstrapUi extends SwaggerBootstrapUiOptions {
    ...
    if (KUtils.checkUndefined(urlParams)) {
      if (urlParams.hasOwnProperty('groupName')) {
        ...
        if (KUtils.checkUndefined(gpName) && gpName != '') {
          const selectInstance = this.selectInstanceByGroupName(gpName)
          this.log('包含分组名称')
  ...
  async analysisApi (instance?: SwaggerBootstrapUiInstance) {
    ...
    try {
      ...
      if (!this.currentInstance?.load) {
        ...
        let reqHeaders = { language: this.settings.language }
        let requestConfig = {
          url: api,
          dataType: 'json',
          timeout: 20000,
  ...
  createDetailMenu (addFlag: boolean) {
    ...
    // 创建菜单数据
    const menuArr : MenuInstance[] = []
    this.log(this.currentInstance)
    const groupName = this.currentInstance.name
    const groupId = this.currentInstance.id
    ...
    if (this.settings.enableSwaggerModels) {
      // 重命名model
      const swaggerModelName = KUtils.getValue(this.settings, 'swaggerModelName', 'Swagger Models', true)
    ...
    const mdata = KUtils.formatter(menuArr, '/', undefined)
    // 添加全局参数
    if (addFlag) {
